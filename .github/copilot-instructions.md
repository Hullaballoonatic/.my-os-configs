...
The macOS host is managed by nix-darwin. Homebrew is integrated through
`nix-homebrew`, adopting the existing ARM Homebrew installation in
`/opt/homebrew`; only applications deliberately retained on Homebrew belong
in the Darwin `homebrew` module. Ghostty is currently one such application
because the pinned nixpkgs version does not provide it for `aarch64-darwin`.
Its configuration is still generated by Home Manager.
...
- Keep shared application configuration in `nix/home/modules`, and use
  host-specific Home Manager files only for platform or host differences.
- Nushell receives its PATH from `home.sessionPath` through its generated
  `env.nu`; do not duplicate PATH values in terminal or application configs.
- Herdr's configuration is generated by Home Manager. Its running server must
  be reloaded with `herdr server reload-config`, or restarted when changing
  startup-only settings.
- Programming language runtimes and package managers should generally move
  from global installation into the appropriate `nix/templates` development
  shell after existing projects and IDE workflows have been inventoried.
